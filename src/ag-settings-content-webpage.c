...
G_DEFINE_TYPE(AgSettingsContentWebpage, ag_settings_content_webpage, GTK_TYPE_SCROLLED_WINDOW)

static void ag_settings_content_webpage_init(AgSettingsContentWebpage* self)
{
    // Self properties
    gtk_container_set_border_width(GTK_CONTAINER(self), 5);

    // Load elements from resources
    GtkBuilder* builder = gtk_builder_new_from_resource("/net/bartkessels/apagenerator/settings_webpage.ui");
...
static void ag_settings_content_webpage_class_init(AgSettingsContentWebpageClass* class)
{
...
{
    return g_object_new(AG_TYPE_SETTINGS_CONTENT_WEBPAGE, NULL);
}